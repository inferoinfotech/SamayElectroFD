...
import { downloadCSVTemplate, uploadLoggerCSV } from "@/api/meterData";
import norecord from "/images/No data-pana.webp";

export default function UploadLoggerFile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await uploadLoggerCSV(
        files,
        parseInt(month),
        parseInt(year)
      );
      ...
      if (response.results && response.results.length > 0) {
        ...
        const errorMessages = formatErrorMessage(response.message);
        setUploadErrors(errorMessages);
        removeAllFiles();
        setMonth("");
        setYear("");
      } else {
        toast.warning("Files were uploaded but no data was processed.");
        removeAllFiles();
        setMonth("");
        setYear("");
      }
    } catch (error) {
      const errorMessages = formatErrorMessage(error.message || error);
      setUploadErrors(errorMessages);
      removeAllFiles();
    } finally {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        link.href = url;
        link.setAttribute('download', 'logger_template.csv');
  ...
  return (
    <div className="mx-auto py-4 px-4 flex flex-col lg:flex-row items-start gap-6">
      ...
      <Card className="w-full lg:w-1/2 shadow-lg border-t-4 border-t-primary">
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Button
              ...
              className="flex items-center border-slate-400 font-semibold bg-gray-100 hover:cursor-pointer gap-2 w-full sm:w-auto"
            >
              {isDownloading ? (
                <>
                  <svg
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-grow" key={formKey}>
          <CardContent className="space-y-6 flex-grow">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger id="month" className="bg-background border-gray-600 cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                <Select value={year} onValueChange={setYear}>
                  <SelectTrigger id="year" className="bg-background border-gray-600 cursor-pointer">
            ...
            <div className="space-y-3">
              ...
              <div
                className={`mt-1 flex justify-center px-4 sm:px-6 pt-5 pb-6 border-2 border-dashed rounded-md transition-colors ${files.length > 0
            ...
            {uploadErrors.length > 0 && (
              <div className="mt-4 p-3 sm:p-4 bg-destructive/10 rounded-md border border-destructive/20">
                <div className="flex items-center gap-2 text-destructive mb-2">
                  <AlertCircle className="h-4 w-4" />
                  <h3 className="font-medium text-sm sm:text-base">
                    The following files already have data for this month:
                  </h3>
                </div>
        ...
        <CardContent className="space-y-2 overflow-y-auto history-scrollbar flex flex-col items-center justify-center flex-grow h-full">
          {files.length > 0 ? (
            <div className="mt-4 animate-in fade-in-50 duration-300 space-y-2 w-full">
              ...
              <div className="space-y-2 overflow-y-auto history-scrollbar grid grid-cols-1 sm:grid-cols-2 gap-2">
                {files.map((file, index) => (
                  <div
                    key={index}
                    className="bg-background border rounded-lg p-3 m-0"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center min-w-0"> {/* Added min-w-0 to handle text overflow */}
